...
def merge_single_term(text, merge_text, client):
    concept = client.get('concepts', concept_selector={'type': 'specified',
                         'concepts': [{'texts': [text]}]})
    merge_concept = client.get('concepts', concept_selector={
                               'type': 'specified',
                               'concepts': [
                                   {'texts': [merge_text]}
                               ]})
    term = concept['result'][0]['exact_term_ids'][0]
    client.put('terms/manage', term_management={
                term: {'new_term_id':
                       merge_concept['result'][0]['exact_term_ids'][0]}})
...
def merge_multiple_terms(texts, merge_text, client):
    ...
    concept = client.get('concepts',
                         concept_selector={'type': 'specified', 
                            'concepts': concepts})['result']
    ignore_terms = [c['exact_term_ids'][0] for c in
                    concept if len(c['exact_term_ids']) > 0]
    merge_concept = client.get('concepts', 
                               concept_selector={'type': 'specified',
                               'concepts': [{'texts': [merge_text]}]})
    terms = {t: {'new_term_id': 
                 merge_concept['result'][0]['exact_term_ids'][0]}
...
def main():
    ...
    parser.add_argument('merge_into_text',
                        help="Text of the Concept to merge the following"
                             " concepts into")
    parser.add_argument('-m', '--merge_from_text', default=None,
                        help="Text to roll into merge_text in project")
    parser.add_argument('-f', '--filename', default=None,
                        help="Name of the file to read list of text to merge")
    args = parser.parse_args()
    
    endpoint = parse_url(args.project_url)
    client = LuminosoClient.connect(endpoint,
                                    user_agent_suffix='se_code:merge_terms')

    if args.merge_into_text and args.merge_from_text:
        term_manage = merge_single_term(args.merge_from_text,
                                        args.merge_into_text, client)
        ...
        term_manage = merge_single_term(merge_from_text, merge_into_text, 
                                        client)
