...
class SentimentTopics:
    ...
    def _get_known_sentiment_terms(self, sentiment):
        ...
        if sentiment == "pos":
            return [
                term for term in self.project_terms if term["orig-sentiment-score"] > 0
            ]
        elif sentiment == "neg":
            return [
                term for term in self.project_terms if term["orig-sentiment-score"] < 0
    ...
    def get_domain_sentiment_terms(self, sentiment):
        ...
        terms = self.client.get(
            "terms/search",
            vector=pack64(axis),
            limit=self.n_results if self.n_results > 200 else 200,
        )["search_results"]
    ...
    def sorted_sentiment_terms(self, sentiment):
        ...
        return sorted(sentiment_terms, key=self._harmonic_mean, reverse=True)[
            : self.n_results
        ]
    ...
    def _normalize_scores(self, terms):
        ...
        for term in terms:
            term["norm-new-sent-score"] = self._normalize_score(
                term["new-sentiment-score"], all_sentiment_scores
            )
            term["norm-relevance-score"] = self._normalize_score(
                term["score"], all_relevance_scores
            )
...
def print_sentiment_terms(terms, verbose=False):
    ...
    for term in terms:
        ...
        if verbose:
            output += "\t{}\t{}".format(
                term["norm-new-sent-score"], term["norm-relevance-score"]
            )
        print(output)
...
@click.option(
    "--n-terms",
    default=500,
    help="Number of project terms among which to find terms "
...
def main(
    ...
    if terms:
        print_sentiment_terms(
            sentiment_topics.sorted_sentiment_terms(sentiment), verbose=verbose
        )
