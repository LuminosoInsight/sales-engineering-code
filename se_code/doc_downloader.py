...
def add_sentiment_to_saved_concepts(client, saved_concepts):
    saved_concept_ids = [{"saved_concept_id":c['saved_concept_id']} for c in saved_concepts]
    concept_selector = {"type": "specified", "concepts": saved_concept_ids}
    saved_sentiments = client.get('concepts/sentiment', concept_selector=concept_selector)
    for sc, match_count in zip(saved_concepts, saved_sentiments['match_counts']):
        ...
        sc['sentiment'] = collections.Counter(match_count['sentiment_share']).most_common()[0][0]

def add_relations(client, docs, add_concept_relations=False, add_concept_list=False, match_type="both", add_match_score=False, add_saved_concept_sentiment=False):
        ...
        for sc in saved_concepts:
            if (d['doc_id'] in sc['docs_ids']):
                ...
                if add_saved_concept_sentiment:
                    d['metadata'].append({'name':sc['name']+' sentiment','type':'string','value':sc['sentiment']})
            else:
                v = 'no'
                if add_concept_relations:
                    d['metadata'].append({'name':sc['name'],'type':'string','value':v})
...
def main():
    ...
    parser.add_argument('-mt', '--match_type', default=None, help="For concept relations use exact, conceptual or both when searching")
    parser.add_argument('-ms', '--match_score', default=False, action='store_true', help="For concept relations also include the match_score")
    parser.add_argument('-s', '--concept_relations_sentiment', default=False, action="store_true", help="Add saved concept sentiment to the concept_relations")
    
    args = parser.parse_args()

    # require that -c is set if using any of the other flags associated with concept relations
    if (args.concept_list or args.match_score or args.concept_relations_sentiment or args.match_type!=None) and (not args.concept_relations):
    ...
    if args.concept_relations or args.concept_list:
        add_relations(client,docs,args.concept_relations,args.concept_list,match_type=args.match_type,add_match_score=args.match_score, add_saved_concept_sentiment=args.concept_relations_sentiment)
    fields = get_fields(docs)
    docs, field_names = format_subsets(docs, fields, args.date_format)

    # need a better sort to the field names
    field_name_dict = {fn:fn.replace("string_","").replace("number_","").replace("score_","").replace("text","1text").replace("title","1title") for fn in field_names}
    field_names = {k: v for k, v in sorted(field_name_dict.items(), key=lambda item: item[1])}.keys()
